...
if [[ "${SERVICE_ENV}" = "gcloud" ]]; then
  ...
  java -jar /app/*.jar --spring.datasource.url=jdbc:postgresql://host.docker.internal:5432/core_finance_product --spring.config.location="file:///app/application.yaml" ${GENERIC_APP_ARG} ${RUN_ENV}
else
